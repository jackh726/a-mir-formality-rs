...
pub fn instantiate_qualified<T: Clone + Subst>(
    env_0: Env,
    ...
    match quantifier {
        Quantifier::ForAll => {
            ...
            let vars = KindedVarIds(
                kinded_var_ids
                    .0
                    .into_iter()
                    .zip(varids_new.0.clone().into_iter())
                    .map(|(kv, var_ids)| KindedVarId(kv.0, var_ids))
                    .collect(),
            );
            let env_2 = utils::env_with_vars_in_current_universe(env_1, Quantifier::ForAll, vars);

            let term_1 = utils::apply_substitution(
                Substitution(
                    subst_to_placeholders
                        .0
                        .into_iter()
                        .map(|pair| (pair.0, Parameter::Ty(Ty::VarId(pair.1))))
                        .collect(),
                ),
                term_0,
            );
